...
Батчевый пайплайн данных, который ежедневно получает курсы валют с API
Центрального банка РФ, сохраняет их в исходном виде, преобразует в
удобный для запросов формат и запускается по расписанию с автоматической
проверкой данных.
...
| Land        | Сохранение сырого, неизменённого JSON-ответа                             | таблица `raw_daily_rates`         |
| Transform   | Разбор JSON в типизированные построчные данные по каждой валюте          | `transform/staging_rates.sql`     |
| Guard       | Явный отказ (fail), если данных мало или они устарели                    | `sql/guard_check.sql`             |
| Orchestrate | Ежедневный запуск ingest → transform → guard с контролем зависимостей    | DAG Airflow (`dags/valute_pipeline.py`) |
